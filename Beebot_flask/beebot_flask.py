...
def get_bot_response():
    ...
    result = request.form
    user_input = result['msg']

    # Quitter
    if re.search(quit_user, user_input) :
        return f'Beebot : {random.choice(quit_bot)}' 
        isawake = False

    # Fonctionnalités du bot
    elif re.fullmatch(fct_user, user_input) :
        return f'beebot : {fct_bot}'
    ...
    # Salutations
        ...
        if CurrentHour >= 6 and CurrentHour < 18:
                return render_template('/index.html', reponse = f'beebot : Bonjour !')
        elif CurrentHour >= 18 and CurrentHour != 6:
                return  render_template('/index.html', reponse = f'beebot : Bonsoir !')
        
    # Nom du bot
    elif re.search(botname_user, user_input) :
        return  render_template('/index.html', reponse = f'Beebot : {random.choice(botname_bot)}, et toi ?')
        user_name = input('user : ').upper()
        return  render_template('/index.html', reponse = f'Beebot : Enchanté {user_name} !')

    # la forme ?
    elif re.search(howis_user, user_input) :
        return  render_template('/index.html', reponse = f'Beebot : {random.choice(howis_bot)}, et toi')
        user_howis = input('user :').lower()
        txtblob = TextBlob(user_howis).sentiment.polarity
        if txtblob > 0.00 :
                return  render_template('/index.html', reponse = f'Beebot : Yeaah {user_name} !!! ça fait du bien de voir des gens avec le sourire !!!')
        elif txtblob < 0.00 :
                return  render_template('/index.html', reponse = f"Beebot : Courage {user_name}, plus que 34 785 heures de confinement et c'est terminado...")
        else :
                 return  render_template('/index.html', reponse = f"Beebot : Resaisis-toi {user_name} ! ça aurait puêtre pire ! Tu aurais pu être coincé sous le clavier...")

    # Age
    elif re.search(botage_user, user_input) :
        return  render_template('/index.html', reponse = f'Beebot : {random.choice(botage_bot)}')

    # Chanson
    elif re.search(song_user, user_input) :
        user_input = re.sub(f"[{string.punctuation}]", " ", user_input)
        return  render_template('/index.html', reponse = f'Beebot : {random.choice(song_bot)}')
    ...
    # Module Météo
        ...
        return  render_template('/index.html', reponse = f'Beebot : Il fait {temp} degrés à {ville},\nTemps {temps}')
        ...
        return render_template('/index.html', reponse = f'Beebot : Je suis à {city}, en {country}')

    elif re.search(doin_user, user_input) :
        user_input = re.sub(f"[{string.punctuation}]", " ", user_input)
        return render_template('/index.html', reponse = f'Beebot : {random.choice(doin_bot)}')
        ...
        return render_template('/index.html', reponse = f'Beebot : Il est {heure}')

    else :
        return render_template('/index.html', reponse = f'Beebot : {random.choice(misunderstood)}')
